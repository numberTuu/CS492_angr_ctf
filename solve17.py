...
def main(argv):
  ...
  def has_active():
    ...
    # Basically checking if I still have an active state to explore or not
  ...
  while (has_active() or has_unconstrained()) and (not has_found_solution()):
    ...
    for unconstrained_state in simulation.unconstrained:
      ...
      # (!)
      if unconstrained_state.satisfiable(extra_constraints=[eip == 0x414B4829]):
        # We can!
        solution_state = unconstrained_state 	# solution_state is no longer None

        # Now, constrain eip to equal the address of the print_good function.
        # (!)
        solution_state.add_constraints(solution_state.regs.eip == 0x414B4829)
  ...
  if solution_state:
    ...
    for byte in solution_state.posix.files[sys.stdin.fileno()].all_bytes().chop(bits=8):
      solution_state.add_constraints(byte >= 0x29, byte <= 'Z')
